...
# BEGIN PATCH AMOCO
from amoco.arch.sparc.parsers import env

from amoco.cas.expressions import cst, op, comp
spec_table = dict([(spec.iattr['mnemonic'], spec) for spec in amoco.arch.sparc.spec_v8.ISPECS ])
b_synonyms = {'b':'ba','bgeu':'bcc','blu':'bcs','bz':'be','bnz':'bne'}
t_synonyms = {'t':'ta','tgeu':'tcc','tlu':'tcs','tz':'te','tnz':'tne'}
b_cond = dict([(mn,cond) for cond,mn in amoco.arch.sparc.formats.CONDB.items()])
t_cond = dict([(mn,cond) for cond,mn in amoco.arch.sparc.formats.CONDT.items()])
def asmhelper(i):
    ...
    return i
amoco.arch.sparc.parsers.asmhelper = asmhelper
# END PATCH AMOCO
